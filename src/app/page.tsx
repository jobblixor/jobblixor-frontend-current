...
export default function Page() {
  ...
  // --- FORM SUBMISSION: Save to Firebase + localStorage + chrome.storage.local ---
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // NEW: Save email to chrome.storage.local for extension to read (cross-domain)
      if (
        ...
        window.chrome.storage.local.set({ email: formEmail }, function() {
          console.log("Jobblixor: Saved email to chrome.storage.local:", formEmail);
        });
      }
