...
async function startAutoApplying() {
  ...
  try {
    if (!email || !password) {
      ...
      debugLog("Invalid email or password provided:", email);
      return;
    }

    // Load user profile with auth
    if (!userProfile) {
      const authSuccess = await loadUserProfile(email, password);
      if (!authSuccess) return;  // Stop if auth failed
    }
    ...
    if (currentUrl.includes('smartapply.indeed.com') || currentUrl.includes('us.smartapply.indeed.com')) {
      ...
      // CRITICAL: Send to background script FIRST to save state to storage
      debugLog("Sending START_AUTO_APPLY to background script to persist state");
      chrome.runtime.sendMessage(message, (bgResponse) => {
        debugLog("Background script acknowledged START_AUTO_APPLY:", bgResponse);
      });
      
      // Then send to content script to start orchestrator
      const response = await sendMessageWithTimeout(tab.id, message, 10000);
...
// Replace loadUserProfile and add authentication
async function authenticateAndLoadProfile(email, password) {
  ...
  try {
    ...
    debugLog("Authentication successful, UID:", uid);

    // Store both email and UID
    localStorage.setItem('jobblixor_email', email);
    localStorage.setItem('jobblixor_uid', uid);

    // Store UID in chrome.storage too
    await chrome.storage.local.set({ 'jobblixor_uid': uid });
    ...
    if (response?.success && response.data) {
      ...
      updateDisplay();
      updateStatus('Profile loaded successfully! Ready to start.');
      return true;
    } else {
...
async function loadUserProfile(email, password) {
  if (!password) {
    updateStatus('Password required');
    return false;
  }
  return await authenticateAndLoadProfile(email, password);
}
...
function clearAllAutomationState() {
  debugLog("CRITICAL CLEANUP: Clearing all automation state and explicitlyStarted flag");
  
  try {
    // Clear localStorage keys
    localStorage.removeItem(STATE_STORAGE_KEY);
    localStorage.removeItem(AUTOMATION_STATE_KEY);
    localStorage.removeItem('jobblixor_automation_state');
    localStorage.removeItem('jobblixor_current_state');
    
    // Also clear chrome.storage.local keys
    chrome.storage.local.remove([
      'jobblixor_phase2_paused',
      'jobblixor_orchestrator_tab', 
      'jobblixor_phase2_running',
      'jobblixor_automation_state',
      'jobblixor_current_state'
    ], () => {
      debugLog("chrome.storage.local automation keys cleared");
    });
    
    debugLog("âœ… COMPLETE CLEANUP: All automation state cleared - explicitlyStarted flag reset");
  } catch (error) {
